...
void get_gf_force(const GaugeField<group> &U, const plaqw_t<wT> &plaqw,
...
#if GFLOWS == 1 // Bulk-Prevention (BP)
    get_force_bp(U, plaqw, E, eps);
#elif GFLOWS == 2 // Luscher-Weisz (LW)
    atype c12 = -1.0 / 12.0;     // rectangle weight
    atype c11 = 1.0 - 8.0 * c12; // plaquette weight
    get_force_impr(U, plaqw, E, eps * c11, eps * c12);
#elif GFLOWS == 3 // IWASAKI
    atype c12 = -0.331;          // rectangle weight
    atype c11 = 1.0 - 8.0 * c12; // plaquette weight
    get_force_impr(U, plaqw, E, eps * c11, eps * c12);
    ...
    get_force_impr(U, plaqw, E, eps * c11, eps * c12);
#elif GFLOWS == 5 // LOG-PLAQUETTE
    get_force_log_plaq(U, plaqw, E, eps);
#else             // WILSON
    get_force_wplaq(U, plaqw, E, eps);
#endif
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
atype measure_gf_s(const GaugeField<group> &U, const plaqw_t<wT> &plaqw) {
    // wrapper for gauge action computation routine
    // (for gauge action that is used to evolve the flow)
#if GFLOWS == 1 // BP
    atype res = measure_s_bp(U, plaqw);
#elif GFLOWS == 2 // LW
    atype c12 = -1.0 / 12.0;     // rectangle weight
    atype c11 = 1.0 - 8.0 * c12; // plaquette weight
    atype res = measure_s_impr(U, plaqw, c11, c12);
#elif GFLOWS == 3 // IWASAKI
    atype c12 = -0.331;          // rectangle weight
    atype c11 = 1.0 - 8.0 * c12; // plaquette weight
    atype res = measure_s_impr(U, plaqw, c11, c12);
    ...
    atype res = measure_s_impr(U, plaqw, c11, c12);
#elif GFLOWS == 5 // LOG-PLAQUETTE
    atype res = measure_s_log_plaq(U, plaqw);
#else             // WILSON
    atype res = measure_s_wplaq(U, plaqw);
#endif
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
atype measure_dE_wplaq_dt(const GaugeField<group> &U, const plaqw_t<wT> &plaqw) {
    Reduction<double> de = 0;
    de.allreduce(false).delayed(true);
    VectorField<Algebra<group>> K, Kc;
    get_gf_force(U, plaqw, K);
    get_force_wplaq(U, plaqw, Kc, -2.0);
    foralldir(d) {
        onsites(ALL) {
            de += Kc[d][X].dot(K[d][X]);
        }
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
atype measure_dE_clov_dt(const GaugeField<group> &U, const plaqw_t<wT> &plaqw) {
    Reduction<double> de = 0;
    de.allreduce(false).delayed(true);
    VectorField<Algebra<group>> K, Kc;
    get_gf_force(U, plaqw, K);
    get_force_clover(U, plaqw, Kc, -2.0);
    foralldir(d) {
        onsites(ALL) {
            de += Kc[d][X].dot(K[d][X]);
        }
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
atype measure_dE_log_dt(const GaugeField<group> &U, const plaqw_t<wT> &plaqw) {
    Reduction<double> de = 0;
    de.allreduce(false).delayed(true);
    VectorField<Algebra<group>> K, Kc;
    get_gf_force(U, plaqw, K);
    get_force_log(U, plaqw, Kc, -2.0);
    foralldir(d) {
        onsites(ALL) {
            de += Kc[d][X].dot(K[d][X]);
        }
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
void measure_gradient_flow_stuff(const GaugeField<group> &V, const plaqw_t<wT> &plaqw, atype flow_l,
                                 atype t_step) {
    // perform measurements on flowed gauge configuration V at flow scale flow_l
    // [t_step is the flow time integration step size used in last gradient flow step]
    // and print results in formatted form to standard output
    ...
    atype slocal = measure_gf_s(V, plaqw) /
                   (lattice.volume() * NDIM * (NDIM - 1) / 2); // average action per plaquette

    atype max_plaq = 0;
    atype plaq = measure_s_wplaq(V, plaqw, max_plaq) /
                 (lattice.volume() * NDIM * (NDIM - 1) / 2); // average wilson plaquette action
    atype eplaq = plaq * NDIM * (NDIM - 1) *
                  group::size(); // naive energy density (based on wilson plaquette action)
    ...
    // clover definition of field strength tensor :
    atype qtopocl, ecl;
    measure_topo_charge_and_energy_clover(V, plaqw, qtopocl, ecl);
    ecl /= lattice.volume();

    // average energy density and toplogical charge from
    // symmetric log definition of field strength tensor :
    atype qtopolog, elog;
    measure_topo_charge_and_energy_log(V, plaqw, qtopolog, elog);
    elog /= lattice.volume();

    // derivative of plaquette energy density w.r.t. to flow time :
    atype deplaqdt = measure_dE_wplaq_dt(V, plaqw) / lattice.volume();

    // derivative of clover energy density w.r.t. to flow time :
    atype declovdt = measure_dE_clov_dt(V, plaqw) / lattice.volume();

    // derivative of log energy density w.r.t. to flow time :
    atype delogdt = measure_dE_log_dt(V, plaqw) / lattice.volume();
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
atype do_gradient_flow_adapt(GaugeField<group> &V, const plaqw_t<wT> &plaqw, atype l_start,
                             atype l_end, atype atol = 1.0e-6, atype rtol = 1.0e-4,
                             atype tstep = 0.0) {
    // wilson flow integration from flow scale l_start to l_end using 3rd order
    // 3-step Runge-Kutta (RK3) from arXiv:1006.4518 (cf. appendix C of
    // arXiv:2101.05320 for derivation of this Runge-Kutta method)
    ...
    while (t < tmax && !stop) {
        ...
        get_gf_force(V, plaqw, k1);
        foralldir(d) onsites(ALL) {
            // first steps of RK3 and RK2 are the same :
            V[d][X] = chexp(k1[d][X] * (step * a11)) * V[d][X];
        }

        get_gf_force(V, plaqw, k2);
        foralldir(d) onsites(ALL) {
            // second step of RK2 :
            // (tk[d][X] will be used for rel. error computation)
            tk[d][X] = k2[d][X];
