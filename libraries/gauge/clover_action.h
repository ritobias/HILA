...
void get_clover_leaves(const GaugeField<group> &U, const plaqw_t<wT> &plaqw, Direction d1,
                       Direction d2, Field<group>(out_only &C)[4], out_only Field<group> &Csum) {
    // assignes the clover leaf matrices in counter-clockwise order to C[0],C[1],C[2],C[3] and sets
    // Csum to the full Clover matrix, i.e. to the anti-hermitian tracless part of
    // (C[0]+C[1]+C[2]+C[3])/4
    ...
    onsites (ALL) {
        // d1-d2-plaquette that starts and ends at X
        tF[X] = conj(plaqw[d1][d2][X]) * U[d2][X + d1] * (U[d2][X] * U[d1][X + d2]).dagger();
        tF[X] *= 0.25;
        C[0][X] = U[d1][X] * tF[X];
...
double measure_s_clover(const GaugeField<group> &U, const plaqw_t<wT> &plaqw) {
    // measure the clover action for dir1<dir2
    // (just to have same normalization as with plaquette action)
    Reduction<double> stot = 0;
    stot.allreduce(false).delayed(true);
    ...
    foralldir (dir1)
        foralldir (dir2)
            if (dir1 < dir2) {
                get_clover_leaves(U, plaqw, dir1, dir2, C, Csum);
                onsites (ALL) {
                    stot += 0.5 * Csum[X].squarenorm();
                }
            }
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
void get_force_clover_add(const GaugeField<group> &U, const plaqw_t<wT> &plaqw, VectorField<Algebra<group>> &K,
                          atype eps = 1.0) {
    // compute gauge force for clover action and add result to K
    Field<group> C[4];
    Field<group> Csum;
    ...
    foralldir (dir1)
        foralldir (dir2)
            if (dir1 < dir2) {
                // get the 4 clover leaf matrices C[][X] (ordered counter-clockwise in
                // (dir1,dir2)-plane) and their anti-hermitian traceless sum Csum[X]
                get_clover_leaves(U, plaqw, dir1, dir2, C, Csum);
                ...
                for (int k = 0; k < 4; ++k) {
                    // multiply the clover leave matrix C[k][X] by Csum[X] --> yields (weighted) sum
                    // of Wilson loops with foot point X and whose first 4 links are always those of
                    // C[k][X]
                    onsites (ALL) {
                        tC[X] = C[k][X] * Csum[X];
                    }
                    // compute the gauge force from the first 4 links of all the Wilson loops that
                    // are summed in tC[k] (is valid to do so since first 4 links are the same for
                    // all Wilson loops and projection on Lie-algebra is linear)
                    get_wloop_force_from_wl_add(U, paths[k], tC, eps, K);
                }
            }
}
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
void get_force_clover(const GaugeField<group> &U, const plaqw_t<wT> &plaqw, out_only VectorField<Algebra<group>> &K,
                      atype eps = 1.0) {
    // determine gauge force for clover action and store result in K
    foralldir (d1) {
        K[d1][ALL] = 0;
    }
    get_force_clover_add(U, plaqw, K, eps);
}
