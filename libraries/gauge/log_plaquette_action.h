...
void get_log_plaq_mat(const GaugeField<group> &U, const plaqw_t<wT> &plaqw, Direction dir1,
                      Direction dir2, out_only Field<group> &C) {
    // sets C=log(U) where U is the plaquette variable panned by dir1 and dir2
    U[dir2].start_gather(dir1, ALL);
    U[dir1].start_gather(dir2, ALL);
    ...
    onsites (ALL) {
        // log of dir1-dir2-plaquette that starts and ends at X; corresponds to F[dir1][dir2]
        // at center location X+dir1/2+dir2/2 of plaquette:
        C[X] = log((conj(plaqw[dir1][dir2][X]) * U[dir1][X] * U[dir2][X + dir1] *
...
double measure_s_log_plaq(const GaugeField<group> &U, const plaqw_t<wT> &plaqw) {
    // measure the log action for dir1<dir2
    // (just to have same normalization as with plaquette action)
    Reduction<double> stot = 0;
    stot.allreduce(false).delayed(true);
    ...
    foralldir (dir1)
        foralldir (dir2)
            if (dir1 < dir2) {
                get_log_plaq_mat(U, plaqw, dir1, dir2, C);
                onsites (ALL) {
                    stot += 0.5 * C[X].squarenorm();
                }
            }
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
void get_force_log_plaq_add(const GaugeField<group> &U, const plaqw_t<wT> &plaqw,
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
void get_force_log_plaq(const GaugeField<group> &U, const plaqw_t<wT> &plaqw,
                        out_only VectorField<Algebra<group>> &K, atype eps = 1.0) {
    // determine gauge force for clover action and store result in K
    foralldir (d1) {
        K[d1][ALL] = 0;
    }
    get_force_log_plaq_add(U, plaqw, K, eps);
}
