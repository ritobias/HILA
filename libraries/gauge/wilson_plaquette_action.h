...
void rstaplesum(const GaugeField<T> &U, out_only Field<T> &staples, Direction d1,
    ...
    foralldir (d2)
        if (d2 != d1) {
            ...
            // calculate first lower 'u' of the staple sum
            onsites (ALL) {
                lower[X] = plaqw[d1][d2][X] * (U[d1][X] * U[d2][X + d1]).dagger() * U[d2][X];
            }
            lower.start_gather(-d2, ALL);
            ...
            if (first) {
                onsites (ALL) {
                    staples[X] = conj(plaqw[d1][d2][X]) * U[d2][X + d1] *
...
double measure_s_wplaq(const GaugeField<group> &U, const plaqw_t<wT> &plaqw) {
    // measure the total Wilson plaquette action for the gauge field U
    Reduction<double> plaq = 0;
    plaq.allreduce(false).delayed(true);
    foralldir (dir1)
    ...
    foralldir (dir1)
        foralldir (dir2)
            if (dir1 < dir2) {
                ...
                U[dir1].start_gather(dir2, ALL);
                onsites (ALL) {
                    P[X] = 1.0 - real(conj(plaqw[dir1][dir2][X]) *
                                      trace(U[dir1][X] * U[dir2][X + dir1] *
                                            (U[dir2][X] * U[dir1][X + dir2]).dagger())) /
                                     group::size();
                    plaq += (double)P[X];
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
void get_force_wplaq_add(const GaugeField<group> &U, const plaqw_t<wT> &plaqw,
                         VectorField<Algebra<group>> &K, atype eps = 1.0) {
    // compute the force for the plaquette action and write it to K

    Field<group> staple;
    foralldir (d) {
        rstaplesum(U, staple, d, plaqw);
        onsites (ALL) {
            K[d][X] -= (U[d][X] * staple[X]).project_to_algebra_scaled(eps);
        }
    }
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
void get_force_wplaq(const GaugeField<group> &U, const plaqw_t<wT> &plaqw,
