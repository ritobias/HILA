...
void get_log_clover_mat(const GaugeField<group> &U, const plaqw_t<wT> &plaqw, Direction dir1,
                        Direction dir2, out_only Field<group> &Csum) {
    // sets Csum=(C[0]+C[1]+C[2]+C[3])/4 where C[0],C[1],C[2],C[3] are the logs of the clover-leaf
    // matrices in counter-clockwise order
    ...
    onsites (ALL) {
        // log of dir1-dir2-plaquette that starts and ends at X; corresponds to F[dir1][dir2]
        // at center location X+dir1/2+dir2/2 of plaquette:
        Csum[X] = log((conj(plaqw[dir1][dir2][X]) * U[dir1][X] * U[dir2][X + dir1] *
...
double measure_s_log(const GaugeField<group> &U, const plaqw_t<wT> &plaqw) {
    // measure the log action for dir1<dir2
    // (just to have same normalization as with plaquette action)
    Reduction<double> stot = 0;
    stot.allreduce(false).delayed(true);
    ...
    foralldir (dir1)
        foralldir (dir2)
            if (dir1 < dir2) {
                get_log_clover_mat(U, plaqw, dir1, dir2, Csum);
                onsites (ALL) {
                    stot += 0.5 * Csum[X].squarenorm();
                }
            }
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
void get_force_log_add(const GaugeField<group> &U, const plaqw_t<wT> &plaqw,
    ...
    foralldir (dir1)
        foralldir (dir2)
            if (dir1 < dir2) {
                // get the 4 clover leave matrices C[][X] (ordered counter-clockwise in
...
template <typename group, typename wT, typename atype = hila::arithmetic_type<group>>
void get_force_log(const GaugeField<group> &U, const plaqw_t<wT> &plaqw, out_only VectorField<Algebra<group>> &K,
                   atype eps = 1.0) {
    // determine gauge force for clover action and store result in K
    foralldir (d1) {
        K[d1][ALL] = 0;
    }
    get_force_log_add(U, plaqw, K, eps);
}
