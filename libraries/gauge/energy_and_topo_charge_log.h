...
void measure_topo_charge_and_energy_log(const GaugeField<group> &U, const plaqw_t<wT> &plaqw,
                                        atype &qtopo_out, atype &energy_out) {
    // measure topological charge and field strength energy of the gauge field, using the
    // matrix logarithms of the plaquettes as components of the field strength tensor
    ...
    foralldir (dir1)
        foralldir (dir2)
            if (dir1 < dir2) {
                ...
                onsites (ALL) {
                    // log of dir1-dir2-plaquette that starts and ends at X; corresponds to
                    // F[dir1][dir2] at center location X+dir1/2+dir2/2 of plaquette:
                    tF0[X] = log((conj(plaqw[dir1][dir2][X]) * U[dir1][X] * U[dir2][X + dir1] *
                ...
                tF0.start_gather(-dir2, ALL);
                onsites (ALL) {
                    // get F[dir1][dir2] at X from average of the (parallel transported)
                    // F[dir1][dir2] from the centers of all dir1-dir2-plaquettes that touch X :
                    F[k][X] =
                        (tF0[X] + U[dir2][X - dir2].dagger() * tF0[X - dir2] * U[dir2][X - dir2]) *
                        0.25;
                }
                ++k;
            }
