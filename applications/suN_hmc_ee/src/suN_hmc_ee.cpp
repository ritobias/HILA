...
void do_interp_hb_trajectory(GaugeField<group> (&U)[2], const PlaquetteField<pT> &plaq_tbc_mode,
                             parameters &p, int interp_dir, out_only atype &ds) {
...
template <typename T, typename pT, typename atype = hila::arithmetic_type<T>>
void do_hbbc_measure(GaugeField<T> (&U)[2], const PlaquetteField<pT> &plaq_tbc_mode,
                     parameters &p) {
    ...
    if (p.alpha == 0) {
        ipdir = 1;
        do_interp_hb_trajectory(U, plaq_tbc_mode, p, ipdir, ds);
    } else if(p.alpha == 1) {
        ipdir = -1;
        do_interp_hb_trajectory(U, plaq_tbc_mode, p, ipdir, ds);
    } else {
        ...
        do_interp_hb_trajectory(U, plaq_tbc_mode, p, ipdir, ds);
        tttime = hila::gettime();
        hila::out0 << string_format("HBBC            % 1d % 0.6e    % 10.5f\n", ipdir, ds,
                                    tttime - ttime);
        ttime = tttime;
        ds = 0;
        U[0] = U_old;
...
#pragma hila safe_access(bcmsid)
    ...
    {
        // mark the "critical" temporal plaquettes (with value 2) that would be affected when the
        // spatial site with bcmsid equal to bcms + 1 were moved from region B to region A:
        ...
        // mark also the links (with value 2) that are part of the "critical" temporal plaquettes
        foralldir(d1) if (d1 != e_t) {
            onsites(ALL) {
                if (plaq_tbc_mode[d1][e_t][X] == 2) {
                    plaq_tbc_mode[d1][d1][X] = 2;
                    plaq_tbc_mode[e_t][e_t][X] = 2;
                }
                if (plaq_tbc_mode[d1][e_t][X - d1] == 2) {
                    plaq_tbc_mode[e_t][e_t][X] = 2;
                }
                if (plaq_tbc_mode[d1][e_t][X - e_t] == 2) {
                    plaq_tbc_mode[d1][d1][X] = 2;
                }
            }
        }
