...
template <typename T, typename pT, typename atype = hila::arithmetic_type<T>>
std::vector<double> measure_s_wplaq_crit(const GaugeField<T> (&U)[2], const PlaquetteField<pT> &plaq_tbc_mode) {
    // measure the total Wilson action of all plaquettes in the "critical region" i.e. the
    // plaquettes that contain a link with tbc_mode = 2 (indicating that they touch a plaquette with
    // tbc_mode = 2); computed this action once, assuming that the plaquettes with tbc_mode = 2
    // belongs to region B (0) and once assuming that the plaquettes with tbc_mode = 2 belong to
    // reagion A (1).
    ReductionVector<double> plaq(2, 0);
    plaq.allreduce(false).delayed(true);
    foralldir(d1) foralldir(d2) if (d1 < d2) {
        onsites(ALL) {
            int ip = plaq_tbc_mode[d1][d2][X];
            int ip1 = plaq_tbc_mode[d1][d1][X];
            int ip2 = plaq_tbc_mode[d2][d2][X];
            int ip1d2 = plaq_tbc_mode[d1][d1][X + d2];
            int ip2d1 = plaq_tbc_mode[d2][d2][X + d1];
            if (ip == 2) {
                plaq[1] += 1.0 - real(trace(U[1][d1][X] * U[1][d2][X + d1] *
                                            (U[1][d2][X] * U[1][d1][X + d2]).dagger())) /
                                     T::size();

                plaq[0] += 1.0 - real(trace(U[0][d1][X] * U[0][d2][X + d1] *
                                            (U[0][d2][X] * U[0][d1][X + d2]).dagger())) /
                                     T::size();
            } else if (ip1 == 2 || ip2 == 2 || ip1d2 == 2 || ip2d1 == 2) {
                if(ip == 1) {
                    double tplaq = 1.0 - real(trace(U[1][d1][X] * U[1][d2][X + d1] *
                                                    (U[1][d2][X] * U[1][d1][X + d2]).dagger())) /
                                             T::size();
                    plaq[0] += tplaq;
                    plaq[1] += tplaq;
                } else if(ip == 0) {
                    double tplaq = 1.0 - real(trace(U[0][d1][X] * U[0][d2][X + d1] *
                                                    (U[0][d2][X] * U[0][d1][X + d2]).dagger())) /
                                             T::size();
                    plaq[0] += tplaq;
                    plaq[1] += tplaq;
