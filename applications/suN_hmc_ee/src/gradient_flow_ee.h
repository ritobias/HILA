...
atype measure_dE_wplaq_dt(const GaugeField<T> (&U)[2], const PlaquetteField<pT> &plaq_tbc_mode,
    ...
    VectorField<Algebra<T>> K, tK;
    get_force_wplaq(U, plaq_tbc_mode, alpha, tK);
    get_gf_force(U, plaq_tbc_mode, alpha, K);
    foralldir(d) {
        onsites(ALL) {
            de += -2.0 * tK[d][X].dot(K[d][X]);
        }
    }
...
void measure_gradient_flow_stuff(const GaugeField<T> (&V)[2],
    ...
    if (first) {
        ...
        hila::out0 << "LGFLMEAS  l(ambda)        S-plaq       dS_crit      S_crit_0      S_crit_1  "
                      "      E_plaq  "
                      "  dE_plaq/dl  "
    ...
    atype dscrit = measure_ds_wplaq_dbcms(V, plaq_tbc_mode);

    // naive energy density: eplaq = 1/V \sum_{x} \sum_{\mu!=\nu} Tr(\id - U_{\mu\nu})
    atype eplaq = plaq * NDIM * (NDIM - 1) * T::size(); 

    // plaquette action per tbc mode:
    auto scrit_l = measure_s_wplaq_crit(V, plaq_tbc_mode);
    ...
    hila::out0 << string_format("GFLMEAS  % 9.3f % 0.6e % 0.6e % 0.6e % 0.6e % 0.6e % 0.6e % 0.6e "
                                "% 0.6e     [%0.3e]    [%0.3e]\n",
                                flow_l, plaq, dscrit, scrit_l[0], scrit_l[1], eplaq,
                                0.25 * flow_l * deplaqdt, ecl, qtopocl, t_step, max_plaq);
}
